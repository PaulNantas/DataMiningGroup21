...
def calculate_intervals(max, min, no_int):
    ...
    for i in range(no_int):
        interval = [min+interval_size*i, min+interval_size*(i+1)]
...
def convert_value_to_intervals(data_set):
    no_intervals = 40
    sr_intervals, ar_intervals, bmi_intervals, fpm_intervals, abr_intervals = set_intervals(data_set, no_intervals, no_intervals, no_intervals, no_intervals, no_intervals)
...
def run_apriori(data_set):
    ...
    data_set = data_set[['Location', 'Dim1ValueCode','AR', 'SR', 'MBMI', 'FPM', 'ABR']]
...
if __name__ == '__main__':
    ...
    convert_value_to_intervals(data_set)
